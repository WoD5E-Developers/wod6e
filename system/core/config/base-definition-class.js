...
export class BaseDefinitionClass {
  ...
  static async initializeLabels() {
    ...
    if (this.modsEnabled && this.defCategory) {
      ...
      modifications = game.settings.get('wod6e', `modified${this.defCategory}`) || {}
      ...
      activeModules.forEach((module) => {
        ...
        if (module.flags.wod6e.modifications && module.flags.wod6e.modifications[this.type]) {
          ...
          modifications = [...modifications, ...module.flags.wod6e.modifications[this.type]]

          // Log the modification data in the console
          console.log(
            `World of Darkness 6th Edition | Modified ${this.defCategory} added by ${module.id}: ${JSON.stringify(module.flags.wod6e.modifications[this.type])}`
          )
        }
      })
    }
