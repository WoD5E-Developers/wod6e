export const _onSyncFromDataItem = async function (event) {
  ...
  const dataItemId = item.getFlag('wod6e', 'dataItemId')

  // Inform the user if the Data Item ID is empty.
  if (!dataItemId) {
    return ui.notifications.info(game.i18n.localize('WOD6E.ITEMS.DataItemIdCannotBeEmpty'))
  }
  ...
  for (const compendium of compendiumsList) {
    ...
    const foundItems = docs.filter((item) => item?.flags?.wod6e?.dataItemId === dataItemId)

    compendiumDataItems.push(...foundItems)
  }

  const worldDataItems = game.items.filter(
    (item) => item.getFlag('wod6e', 'dataItemId') === dataItemId
  )
...
export const _onSyncToDataItems = async function (event) {
  ...
  for (const actor of actorsList) {
    const foundItems = actor.items.filter((item) => item?.flags?.wod6e?.dataItemId === dataItemId)
  ...
  if (totalCount === 0) {
    return ui.notifications.info(
      game.i18n.format('WOD6E.ITEMS.NoItemsFound', {
        dataItemId
      })
    )
  } else {
    ...
    const content = `<p>
      ${game.i18n.format('WOD6E.ITEMS.ConfirmOverwriteString', {
        string: totalCount
      })}
    </p>`
    ...
    const updateItemsConfirmed = await foundry.applications.api.DialogV2.wait({
      window: {
        title: game.i18n.localize('WOD6E.ITEMS.ConfirmOverwrite')
      },
      classes: ['wod6e', system, 'dialog'],
      content,
      modal: true,
      buttons: [
        {
