...
export const _onFrenzyRoll = async function (event) {
  ...
  const content = `<p>
    ${game.i18n.localize('WOD6E.VTM.FrenzyChoiceResistOrGiveIn')}
  </p>`
  ...
  const doFrenzyRoll = await foundry.applications.api.DialogV2.wait({
    window: {
      title: game.i18n.localize('WOD6E.VTM.ResistingFrenzy')
    },
    content,
    modal: true,
    buttons: [
      {
        label: game.i18n.localize('WOD6E.ItemsList.Resist'),
        action: 'resist'
      },
      {
        label: game.i18n.localize('WOD6E.VTM.GiveIn'),
        action: 'give-in'
      }
    ]
  })
  ...
  if (doFrenzyRoll === 'resist') {
    ...
    basicDice = await WOD6E.api.getBasicDice({
    ...
    WOD5eDice.Roll({
      ...
      callback: async (err, result) => {
        if (err) console.log('World of Darkness 6th Edition | ' + err)
        ...
        if (!result.rollSuccessful) {
          ...
          foundry.documents.ChatMessage.implementation.create({
            flags: {
              wod6e: {
                name: game.i18n.localize('WOD6E.VTM.ResistingFrenzyFailed'),
                img: 'systems/wod6e/assets/icons/dice/vampire/bestial-failure.png',
                description: game.i18n.format('WOD6E.VTM.ResistingFrenzyFailedDescription', {
                  actor: actor.name
                })
              }
            }
          ...
          foundry.documents.ChatMessage.implementation.create({
            flags: {
              wod6e: {
                name: game.i18n.localize('WOD6E.VTM.ResistingFrenzySuccess'),
                img: 'systems/wod6e/assets/icons/dice/vampire/bestial-failure.png',
                description: game.i18n.format('WOD6E.VTM.ResistingFrenzySuccessDescription', {
                  actor: actor.name
                })
              }
            }
    ...
    foundry.documents.ChatMessage.implementation.create({
      flags: {
        wod6e: {
          name: game.i18n.localize('WOD6E.VTM.RidingTheWave'),
          img: 'systems/wod6e/assets/icons/dice/vampire/bestial-failure.png',
          description: game.i18n.format('WOD6E.VTM.RidingTheWaveDescription', {
            actor: actor.name
          })
        }
      }
