...
export const _onAddDiscipline = async function (event) {
  ...
  // Secondary variables
  const disciplineList = WOD6E.Disciplines.getList({})

  // Build the options for the select dropdown
  const content = new foundry.data.fields.StringField({
    choices: disciplineList,
    label: game.i18n.localize('WOD6E.VTM.SelectDiscipline'),
    required: true
  }).toFormGroup(
    {},
    {
  ...
  const disciplineSelected = await foundry.applications.api.DialogV2.prompt({
    window: {
      title: game.i18n.localize('WOD6E.VTM.AddDiscipline')
    },
    classes: ['wod6e', 'dialog', 'vampire', 'dialog'],
    content,
    ok: {
